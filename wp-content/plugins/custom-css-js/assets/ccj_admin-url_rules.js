...
jQuery(document).ready(function ($) {
	...
	function elist_arrange(names) {
		...
		for (n in names) {
			...
			if ('anchor-filters' == name) {
				for (f in field)
					elist_add_row(name, elist_get_row(name, {
						'value': field[f]['value'].esc_html().replace(rq, '&quot;'),
						'type': filters[field[f]['type']].esc_html().replace('-', ' ')
					}, editable), field[f]['index']);
	...
	$('#wplnst-cf-new-add').click(function () {
		...
		if ('' === name)
			return;
		elist_add('custom-fields', {
			'name': name,
			'type': $('#wplnst-cf-new-type').val()
		}, ['wplnst-cf-new'], elist_get_row('custom-fields', {
			'name': name.esc_html(),
			'type': $('#wplnst-cf-new-type option:selected').text().esc_html()
		}, true));
	});
	...
	$('#wplnst-af-new-add').click(function () {
		...
		elist_add('anchor-filters', {
			'value': value,
			'type': type
		}, ['wplnst-af-new'], elist_get_row('anchor-filters', {
			'type': $('#wplnst-af-new-type option:selected').text().esc_html(),
			'value': value.esc_html()
		}, true));
	});
	...
	$('#wplnst-ius-new-add').click(function () {
		...
		if ('' === value)
			return;
		elist_add('include-urls', {
			'value': value,
			'type': $('#wplnst-ius-new-type').val()
		}, ['wplnst-ius-new'], elist_get_row('include-urls', {
			'value': value.esc_html(),
			'type': $('#wplnst-ius-new-type option:selected').text().esc_html()
		}, true));
	});
	...
	$('#wplnst-eus-new-add').click(function () {
		...
		if ('' === value)
			return;
		elist_add('exclude-urls', {
			'value': value,
			'type': $('#wplnst-eus-new-type').val()
		}, ['wplnst-eus-new'], elist_get_row('exclude-urls', {
			'value': value.esc_html(),
			'type': $('#wplnst-eus-new-type option:selected').text().esc_html()
		}, true));
	});
	...
	$('#wplnst-hes-new-add').click(function () {
		...
		elist_add(
			...
			elist_get_row('html-attributes', {
				'element': element.esc_html(),
				'having': $('#wplnst-hes-new-have option:selected').text().toLowerCase().esc_html(),
				'att': att.esc_html(),
				'op': op_text.esc_html(),
				'value': value.esc_html()
			}, true)
