...
if ( ! class_exists( 'FlipperCode_Export_Import' ) ) {
	...
	class FlipperCode_Export_Import {
		...
		*/
		...
		function export($action, $asFilename) {
			...
			if ( 'csv' == $action ) {
				...
				if ( ! empty( $this->data ) ) {
					$wcjp_tab = "\t";
					$wcjp_br = "\n";
					$wcjp_xml_writter  = '<?xml version="1.0" encoding="UTF-8"?>'.$wcjp_br;
					$wcjp_xml_writter .= '<items>'.$wcjp_br;

					foreach ( $this->data as $key => $result ) {
						$wcjp_xml_writter .= $wcjp_tab.'<item>'.$wcjp_br;
						foreach ( $result as $node_key => $node_value ) {
							$wcjp_xml_writter .= $wcjp_tab.$wcjp_tab.'<'.$node_key.'>'.htmlspecialchars( stripslashes( $node_value ) ).'</'.$node_key.'>'.$wcjp_br;
						}
						$wcjp_xml_writter .= $wcjp_tab.'</item>'.$wcjp_br;
					}

					$wcjp_xml_writter .= '</items>';
					echo $wcjp_xml_writter;
				}
			} elseif ( 'json' == $action ) {
		 ...
		 */
		public function wcjp_xml_2array($xml) {
	        ...
	        foreach ( $xml->children() as $r ) {
	            ...
	            if ( count( $r->children() ) == 0 ) {
	                ...
	                $arr[ $r->getName() ][] = $this->wcjp_xml_2array( $r );
	            }
	        }
		...
		function import($action, $filename) {
			...
			if ( 'csv' == $action ) {
				...
				$wcjp_xml_datas = simplexml_load_file( $_FILES[ $filename ]['tmp_name'] );
				$file_data = $this->wcjp_xml_2array( $wcjp_xml_datas );
				...
				$file_data = array();
				$wcjp_json_datas = wp_remote_fopen( $_FILES[ $filename ]['tmp_name'] );
				if ( false === $wcjp_json_datas ) {
					$wcjp_json_datas = file_get_contents( $_FILES[ $filename ]['tmp_name'] );
				}
				$file_datas = json_decode( $wcjp_json_datas, true );
			}
